...
Class.create('Entity',{
    ...
    setListeners: function() {
        var me = this;
        if (me.isServer() && Objects['WebServer']) {
            ...
            Objects['WebServer'].on('socket_server_request', function(params) {
               me.onSocketServerRequest(params);
            });
        }
    },
    ...
    onWebServerRequest: function(params) {
        ...
        if (params.object == me.guid.toLowerCase()) {
            if (me[params.req.method.toLowerCase()+'_'+params.action]) {
                ...
                params.res.end(params.req.params.url);
            }
        }
    },

    onSocketServerRequest: function(params) {
        var me = this;
        if (me['socket_'+params.event]) {
            me['socket_'+params.event](params);
        }
    },
    ...
    show: function(template,res,params) {
        var me = this,
            send_params = params;
        send_params.template = template;
        send_params.guid = me.guid;
        if (!send_params.properties) {
            send_params.properties = {};
        };
        send_params.properties.entity = "Objects['"+me.guid+"']";
        var properties_string = [];
        for (var o in send_params.properties) {
            properties_string.push(o+': '+send_params.properties[o]);
        }
        properties_string = '{'+properties_string.join(',')+'}';
        send_params.properties = properties_string;
        send_params.server_ip = ip.address();
        res.render('layout',send_params);
    },

    resetFormValidation: function(formControls) {
        var me = this;
        if (!me.isServer()) {
            for (var k in formControls) {
                formControls[k] = {errorMessage: '',className:'',errorMessageClassName:'hidden',value:formControls[k].value};
            };
            return formControls;
        }
    },
    ...
    validateForm: function(form,template) {
        var me = this;
        var result = {
            error: false,
            error_message: '',
        ...
        for (var o in form) {
            var item_result = me.validateFormItem(o,form[o].value,template);
            if (item_result.inputClassName == 'error') {
                result.error = true;
            }
            result.formControls[o] = item_result;
        }
        return result;
    }
});
